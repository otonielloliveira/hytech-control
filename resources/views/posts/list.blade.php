...
@section('description',
    'Explore todas as postagens do nosso blog com filtros avançados por data, categoria e tipo de
    conteúdo.')
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-lg-2 col-md-6">
                            ...
                            <select class="form-select" id="date_filter" name="date_filter">
                                ...
                                <option value="last_3_months"
                                    {{ request('date_filter') == 'last_3_months' ? 'selected' : '' }}>
                                    Últimos 3 meses
                                </option>
                        ...
                        <div class="col-lg-2 col-md-6">
                            ...
                            <select class="form-select" id="order_by" name="order_by">
                                <option value="published_at"
                                    {{ request('order_by', 'published_at') == 'published_at' ? 'selected' : '' }}>
                                    Data de publicação
                                </option>
                        ...
                        <div class="col-lg-1 col-md-6">
                            ...
                            <select class="form-select" id="order_direction" name="order_direction">
                                <option value="desc"
                                    {{ request('order_direction', 'desc') == 'desc' ? 'selected' : '' }}>
                                    ↓ Desc
                                </option>
                    ...
                    <div>
                        ...
                        <div class="d-flex flex-wrap gap-2 mt-2">
                            ...
                            @if (request('destination') && request('destination') !== 'all')
                                <span class="badge bg-info">Seção:
                                    {{ $destinations[request('destination')] ?? 'N/A' }}</span>
                            @endif
                            @if (request('date_filter'))
                                <span class="badge bg-warning text-dark">Período:
                                    {{ ucfirst(str_replace('_', ' ', request('date_filter'))) }}</span>
                            @endif
                        </div>
                    </div>
        ...
        @else
            <div class="results-info">
                <h6 class="mb-0">
                    ...
                    <small class="text-muted ms-2">• Página {{ $posts->currentPage() }} de
                        {{ $posts->lastPage() }}</small>
                </h6>
            </div>
        @endif
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const filterSelects = document.querySelectorAll(
                '#category, #destination, #date_filter, #order_by, #order_direction');
            filterSelects.forEach(select => {
                select.addEventListener('change', function() {
                    // Se mudou para data customizada, não submeter ainda
                    if (this.id === 'date_filter' && this.value === 'custom') {
                        customDateFilters.style.display = 'flex';
                        toggleCustomDate.innerHTML =
                            '<i class="fas fa-times me-1"></i>Ocultar Data';
                        return;
                    }
