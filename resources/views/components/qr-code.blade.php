...
if (isset($qrCode) && $qrCode) {
    ...
    if (!empty($paymentModel->qr_code_base64)) {
        $qrHtml = '<img src="data:image/png;base64,' . $paymentModel->qr_code_base64 . '" alt="QR Code PIX" class="img-fluid" />';
    ...
    if (!$qrHtml) {
        if (!empty($paymentResult['qr_code_base64'])) {
            $qrHtml = '<img src="data:image/png;base64,' . $paymentResult['qr_code_base64'] . '" alt="QR Code PIX" class="img-fluid" />';
        ...
        } elseif (!empty($paymentResult['qr_code']) && is_array($paymentResult['qr_code']) && !empty($paymentResult['qr_code']['qr_code_image'])) {
            $qrHtml = '<img src="' . ($paymentResult['qr_code']['qr_code_image'] ?? '') . '" alt="QR Code PIX" class="img-fluid" />';
            $pixCode = $paymentResult['pix_code'] ?? $pixCode;
        }
    }
}
    ...
    <div class="qr-code-wrapper">
        ...
        <div class="mb-3">
            ...
            <div class="input-group">
                <input type="text" class="form-control text-center small" id="pixCode" value="{{ $pixCode ?? '' }}" readonly>
                <button class="btn btn-outline-primary" type="button" onclick="copyPixCode(event)" title="Copiar código PIX">
